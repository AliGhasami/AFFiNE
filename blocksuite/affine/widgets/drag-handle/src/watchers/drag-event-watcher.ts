...
export class DragEventWatcher {
  ...
  private readonly _getDropResult = (
    ...
    if (
      dragPayload &&
      dragPayload.bsEntity?.modelIds.includes(dropBlock.model.id)
    ) {
      return null;
    }
    ...
    if (
      ...
      if (this.isVerticalIndicator) {
        ...
        if (parent) {
          ...
          if (parentBlockComponent) block = parentBlockComponent;
        }
      }
        ...
        if (target) {
          if (
            ...
            if (isContainMultiColumn) {
              ...
              if (this.docPreview) {
                this.docPreview.tooltipMessage = t(
                  'not_allow_drop_column_in_block_column'
                ); //"You can not add columns inside another column block ."
              }
            }
  ...
  private readonly _updateDropResult = (
    ...
    if (showVerticalIndicator) {
      ...
      result = Rect.fromLWTH(
        result?.left,
        result?.width,
        dropResult?.modelState.rect.top - 10,
        dropResult?.modelState.rect.height
      );
  ...
  private _makeDraggable(target: HTMLElement) {
    ...
    return std.dnd.draggable<DragBlockEntity>({
      ...
      setDragPreview: ({ source, container, setOffset }) => {
        ...
        if (source.data.bsEntity.type == 'blocks') return;
            ...
            if (isContainMultiColumn) {
              if (
                ...
                if (res) {
                  ...
                  for (
                    let i = 0;
                    i < multiColumnBlock.model.children.length;
                    i++
                  ) {
                    this.host.doc.moveBlocks(
                      [...multiColumnBlock.model.children[i].children],
                      res.model.children[i + 1]
                    );
                  }
            ...
            if (
              ...
              for (let i = 0; i < multiColumnBlock.model.children.length; i++) {
                ...
                if (res) {
                  this.host.doc.moveBlocks(
                    [...multiColumnBlock.model.children[i].children],
                    res.children[res.children.length - 1]
                  );
                }
              }

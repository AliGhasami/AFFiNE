...
export class MahdaadEdgelessPanToolButton extends QuickToolMixin(LitElement) {
  ...
  override type: GfxToolsFullOptionValue['type'][] = ['pan']; //'pan'
  ...
  override render() {
    const type = this.edgelessTool?.type;
    const { active } = this;
    const tipInfo = { tip: 'Hand', shortcut: 'H' };
    /* type === 'pan'
        ? { tip: 'Hand', shortcut: 'H' }
